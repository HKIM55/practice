...
# 날짜 컬럼 변환 (YYMMDD → YYYY-MM-DD, 시간 제외)
df['날짜'] = pd.to_datetime(df['날짜'].astype(str), format='%y%m%d', errors='coerce').dt.date

# 날짜 NaT가 아닌 데이터만 남기기
df = df.dropna(subset=['날짜'])

# 단가 (원) 처리: 쉼표 제거 및 숫자 변환
...
if search_keyword:
    ...
    if not result_df.empty:
        # 유효한 데이터만 추출
        display_df = result_df[['날짜', '품목', '세부', '마트', '단가 (원)', '단가 (100 g, ml당 가격)']].dropna(subset=['단가 (원)']).sort_values('날짜').reset_index(drop=True)
        ...
        if not display_df.empty:
            ...
            # Min, Max 포인트 강조
            plt.scatter(min_row['날짜'], min_row['단가 (원)'], color='red', label=f"최소: {min_row['단가 (원)']}원 ({min_row['날짜']})")
            plt.scatter(max_row['날짜'], max_row['단가 (원)'], color='green', label=f"최대: {max_row['단가 (원)']}원 ({max_row['날짜']})")
